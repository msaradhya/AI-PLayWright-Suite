/**
 * Enum representing Student Plans Navigation Tabs
 ...
 */
export interface StudentPlansNavTabInfo {
    readonly value: string;
    readonly listIndex: number;
    readonly key: keyof typeof StudentPlansNavTabs;
}

/**
 * Class providing utility methods for StudentPlansNavTabs
 * Enhanced for Playwright test automation support
 */
export class StudentPlansNavTabsUtils {
    
    private static readonly tabInfoMap: ReadonlyMap<StudentPlansNavTabs, StudentPlansNavTabInfo> = new Map([
        [StudentPlansNavTabs.STUDENTPLANS, { value: 'Student Plans', listIndex: 0, key: 'STUDENTPLANS' }]
    ]);

    private static readonly indexToTabMap: ReadonlyMap<number, StudentPlansNavTabs> = new Map([
        [0, StudentPlansNavTabs.STUDENTPLANS]
    ]);
     ...
     */
    static getValue(tab: StudentPlansNavTabs): string {
     ...
     */
    static getListIndex(tab: StudentPlansNavTabs): number {
        const tabInfo = this.tabInfoMap.get(tab);
        if (!tabInfo) {
            throw new Error(`Invalid tab: ${tab}`);
        }
     ...
     */
    static getTabInfo(tab: StudentPlansNavTabs): StudentPlansNavTabInfo {
        const tabInfo = this.tabInfoMap.get(tab);
        if (!tabInfo) {
            throw new Error(`Invalid tab: ${tab}`);
        }
     ...
     */
    static getAllTabs(): StudentPlansNavTabs[] {
        return Object.values(StudentPlansNavTabs);
    }
     ...
     */
    static getAllTabInfos(): StudentPlansNavTabInfo[] {
     ...
     */
    static getTabByIndex(index: number): StudentPlansNavTabs | undefined {
     ...
     * @returns The navigation tab enum or undefined if not found
     */
    static getTabByValue(value: string): StudentPlansNavTabs | undefined {
     ...
     * @returns CSS selector string
     */
    static getTabSelector(tab: StudentPlansNavTabs): string {
     ...
     */
    static getNextTab(currentTab: StudentPlansNavTabs): StudentPlansNavTabs | undefined {
     ...
     */
    static getPreviousTab(currentTab: StudentPlansNavTabs): StudentPlansNavTabs | undefined {
        const currentIndex = this.getListIndex(currentTab);
        return this.getTabByIndex(currentIndex - 1);
    }
}