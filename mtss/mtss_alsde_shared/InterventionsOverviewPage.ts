...
export class InterventionsOverviewPage extends BasePage {
    ...
    constructor(page: Page) {
        super(page);
        this.pageHeading = page.getByRole('heading', { name: /Intervention Overview/i });
    ...
    /**
     * Wait for the Interventions Overview page to load completely
    ...
    /**
     * Validate that the Interventions Overview Dashboard is loaded properly
     */
    async validateDashboard(): Promise<void> {
        ...
        // Verify specific metrics sections are present (based on the actual page structure)
        await expect(this.page.getByRole('heading', { name: /Progress Monitoring/i })).toBeVisible();
        await expect(this.page.getByRole('heading', { name: /How many students are being served/i })).toBeVisible();
    }
    ...
    async validateAllMetrics(timeout: number = 180000): Promise<void> {
        ...
        // Define the expected 9 metric sections and validate each one
        const expectedMetrics = [
            { name: 'Progress Monitoring', selector: this.page.getByRole('heading', { name: /Progress Monitoring/i }) },
            { name: 'How many students are being served in each level of support?', selector: this.page.getByRole('heading', { name: /How many students are being served in each level of support/i }) },
            { name: 'How many active interventions are there for each type and level?', selector: this.page.getByRole('heading', { name: /How many active interventions are there for each type and level/i }) },
            { name: 'How many active interventions are there for each type and Subtype?', selector: this.page.getByRole('heading', { name: /How many active interventions are there for each type and Subtype/i }) },
            { name: 'How many interventions have been completed for each type and subtype?', selector: this.page.getByRole('heading', { name: /How many interventions have been completed for each type and subtype/i }) },
            { name: 'How many active interventions are students receiving in the current school year?', selector: this.page.getByRole('heading', { name: /How many active interventions are students receiving in the current school year/i }) },
            { name: 'Distribution of students by # of active interventions', selector: this.page.getByRole('heading', { name: /Distribution of students by.*active interventions/i }) },
            { name: 'Percentage of students enrolled in active interventions by grade', selector: this.page.getByRole('heading', { name: /Percentage of students enrolled in active interventions by grade/i }) },
            { name: 'Interventions by Race', selector: this.page.getByRole('heading', { name: /Interventions by Race/i }) }
        ];
        
        console.log('📊 Validating 9 distinct metric sections:');
        ...
        console.log('✅ All 9 metrics validation completed successfully');
    }
